...
export const sendMessageToPhilosopherStream = async (
  ...
  if (forceIndependent) {
    ...
    strategyInstruction = "【強制行動：先回 User，再戰同儕】你「必須」先正面且自然地回答 [User] 的問題。在闡述完你的核心觀點後，再挑選歷史紀錄中「其中一位」哲學家的觀點進行反駁或延伸，藉由打擊對手來強化你的論述。最多只針對一個人。（字數彈性限制：為了兼顧兩者，允許擴展至 4 到 7 句精要論述）";
  }

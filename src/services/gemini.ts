...
export const sendMessageToPhilosopherStream = async (
  ...
  if (forceIndependent) {
    ...
    // 💡 修正點：強制要求「點出名字」以增強沙龍實境感
    strategyInstruction = "【強制行動：先回 User，再戰同儕】你「必須」先正面回答 [User] 的問題。接著，挑選歷史紀錄中「其中一位」同儕進行反駁或延伸。請務必在發言中自然地「點出對方的名字」或「明確引用他的觀點」（例如：『柏拉圖，你所謂的理型不過是...』），讓交鋒的火花具體可見。最多只針對一個人。（字數彈性限制：允許擴展至 4 到 7 句精要論述）";
  }
...
`;
  ...
  // 第二步：定義群組專屬指令 (放置在歷史紀錄之後，利用近因效應徹底解決 Lost in the Middle)
...
`;
  ...
  // 第三步：組合最終 Payload (確保群組時，最高執行指令在最尾端)
